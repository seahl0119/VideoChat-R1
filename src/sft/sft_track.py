...
from src.open_r1.my_qwen_utils2 import process_vision_info
...
def load_json_dataset(train_data_path, eval_data_path, video_folder, preprocessed_data_path=None): # Modified to accept preprocessed_data_path
    def create_dataset_from_json(file_path, split_name):
        ...
        for video_data in data:
            ...
            # video_path = os.path.join(video_folder, video_data['video'])
            ...
            example["preprocessed_path"] = result
        ...
        def __getitem__(self, idx): # Define getitem within the scope where dataset is available
            try:
                ...
                print(e)
                return self.__getitem__([min(idx[0]+1, len(dataset)-1)])
...
def convert_example(example):
    ...
    messages.append({
        "role": "user",
        "content": [
            {"type": "text", "text": QUESTION_TEMPLATE.replace("[OBJECT]", example["object"]).replace("[START]", str(example["gt"][0]))},
            {"type": "video", 
    ...
    answer_text = f'<answer>{example["gt"]}</answer>'
    # <glue>{str(example["glue"])}</glue>
